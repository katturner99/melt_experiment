...
norm = plt.Normalize(all_volumes.min(), 0.21e12)
...
# ----------------------------------------------------------
# Density grid (sigma0) — clean TS formulation
# ----------------------------------------------------------

T_centers = 0.5 * (T_bins[:-1] + T_bins[1:])
S_centers = 0.5 * (S_bins[:-1] + S_bins[1:])

SS, TT = np.meshgrid(S_centers, T_centers)

# Use direct sigma0(SP, theta) for TS diagrams
sigma0 = gsw.sigma0(SS, TT)

# Clean, readable density levels
sigma_levels = np.arange(24.8, 28.2, 0.2)
...
plt.savefig(f"TS_plot_{region}.png", dpi=300, bbox_inches="tight")
plt.show()